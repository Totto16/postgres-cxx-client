...
POSTGRES_CXX_ARRAY(TestEnumArray, "test_enum", TestEnum);

static_assert(IsPostgresCXXArray<TestEnumArray>,
              "TestEnumArray must be a postgres::Array");

static_assert(!IsPostgresCXXArray<TestEnum>,
              "TestEnum must NOT be a postgres::Array");
...
TEST(CommandTest, TimeZone) {
  ...
  ASSERT_EQ(static_cast<int>(TIME_STR_SAMPLE_NANO_TZ.size() + 1),
            cmd.lengths()[0]);
  ASSERT_EQ(0, cmd.formats()[0]);
}
